...
const initialFormValues = { 
  date: "", 
  reference: "", 
  entries: [{ coa: "", dr_amount: "", cr_amount: "" }],
  note: "" // Add note field here
};
...
const validationSchema = Yup.object({
  ...
  entries: Yup.array().of(Yup.object({
    coa: Yup.string().required("Chart of accounts is required"),
    dr_amount: Yup.number().min(0, "Amount must be positive"),
    cr_amount: Yup.number().min(0, "Amount must be positive")
...
const EditJournalVoucher = () => {
  ...
  useEffect(() => {
    axios.get(import.meta.env.VITE_APP_BACKEND_URL + "/general-accounting/journalentries/" + id, { headers: { Authorization: `Bearer ${localStorage.getItem("accessToken")}` } })
      .then((response) => {
        setData(response.data);
        axios.get(import.meta.env.VITE_APP_BACKEND_URL + "/general-accounting/journalentryitems/?journal_entry=" + response.data.id, { headers: { Authorization: `Bearer ${localStorage.getItem("accessToken")}` } })
          .then((response) => {
            setData((prevData) => ({
              ...prevData,
    ...
    <div className="layout">
      <div style={{ background: "white", padding: "15px" }}>
        <Row justify="space-between">
          <Title level={4} style={{ margin: 0 }}>{data.journal_entry_no}</Title>
          <Breadcrumb separator="-">
            <Breadcrumb.Item>Home</Breadcrumb.Item>
            <Breadcrumb.Item>Accounting</Breadcrumb.Item>
            <Breadcrumb.Item>Journal Voucher</Breadcrumb.Item>
            <Breadcrumb.Item>{data.journal_entry_no}</Breadcrumb.Item>
          </Breadcrumb>
        </Row>
      </div>
            ...
            return (
              <FormikForm style={{ padding: "10px" }}>
                <Row gutter={16} style={{ marginBottom: 16 }}>
                  <Col span={12}>
                    <Form.Item label="Date" layout="vertical" required validateStatus={touched.date && errors.date ? "error" : ""} help={errors.date}>
                      <Field name="date">
                        {({ field }) => <Input {...field} type="date" placeholder="Enter Date" />}
                      </Field>
                    </Form.Item>
                  </Col>
                  <Col span={12}>
                    <Form.Item layout="vertical" label="Reference" validateStatus={touched.reference && errors.reference ? "error" : ""} help={errors.reference}>
                      <Field name="reference">
                        {({ field }) => <Input {...field} type="text" placeholder="Enter Reference" />}
                      </Field>
                    </Form.Item>
                  </Col>
                </Row>
                  ...
                  {({ push, remove }) => (
                    <>
                      <Table
                              ...
                              {({ field, meta }) => (
                                <div>
                                  <Select
                                    ...
                                    onChange={value => setFieldValue(`entries.${index}.coa`, value)}
                                    style={{ width: "100%" }}
                        ...
                        columns={[
                          { title: "COA", dataIndex: "coa", key: "coa", width: 430 },
                          { title: "Debit Amount", dataIndex: "dr_amount", key: "dr_amount" },
                          { title: "Credit Amount", dataIndex: "cr_amount", key: "cr_amount" },
                          { title: "Actions", dataIndex: "actions", key: "actions" }
                        ]}
                        pagination={false}
                      />
                      <Button type="dashed" onClick={() => push({ coa: "", dr_amount: "", cr_amount: "" })} style={{ marginTop: 16 }}>
                        Add Entry
                      </Button>
                    </>
                  )}
                ...
                <div style={{ marginTop: 16, marginBottom: 16 }}>
                  <Text strong>Total Balance: </Text> ${totalBalance.toFixed(2)}
                </div>

                <Col span={24}>
                  <Form.Item label="Note" layout="vertical" validateStatus={touched.note && errors.note ? "error" : ""} help={errors.note}>
                    <Field name="note">
                      {({ field }) => <Input.TextArea {...field} type="text" placeholder="Enter Note" />}
                    </Field>
                  </Form.Item>
                </Col>
